...
from keras.layers import Conv2D, MaxPool2D, Flatten, Dense, Input, Subtract, Lambda, Layer
...
class SiameseNetwork:
    ...
    def __init__(self, dataset_path,  learning_rate, batch_size, use_augmentation, learning_rate_multipliers, l2_regularization_penalization, tensorboard_log_path):
        ...
        """
        self.input_shape = (height, width, 1)  # Size of images
        self.model = []
        self.learning_rate = learning_rate
        self.omniglot_loader = OmniglotLoader( dataset_path=dataset_path, use_augmentation=use_augmentation, batch_size=batch_size)
        self.summary_writer = tf.summary.FileWriter(tensorboard_log_path)
        self.__construct_siamese_architecture(learning_rate_multipliers, l2_regularization_penalization)
        ...
        for iteration in range(number_of_iterations):
            ...
            if (iteration + 1) % evaluate_each == 0:
                ...
                with open(f'logs/{logs_filename}.txt', 'a') as file:
                    file.write(str(str(iteration) + ' Iterations computed in: ' + str((end - start)/60) + '\n'+'\n'))
                # klasyfikacja = self.model.predict_classes(images)
                # print(klasyfikacja)
            ...
            if iteration - best_accuracy_iteration > 5000:
                print(
